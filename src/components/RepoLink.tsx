...
export function RepoLink({ repo, className = "", tone = "muted" }: { repo: Repo; className?: string; tone?: "muted" | "ink" }) {
  if (repo.visibility === "private") {
    return (
      <span className={`inline-flex items-center gap-1.5 font-mono text-[0.7rem] uppercase tracking-[0.1em] text-ink-faint ${className}`}>
  ...
  return (
    <a
      ...
      className={`group inline-flex items-center gap-1.5 font-mono text-[0.7rem] uppercase tracking-[0.1em] transition-colors hover:text-brass ${
