            ...
            <Reveal as="li" key={job.company} className="relative grid gap-6 border-t border-line py-10 lg:grid-cols-12 lg:gap-10 lg:py-14">
              <div className="lg:col-span-4">
                <div className="lg:sticky lg:top-28">
                  <p className="font-mono text-[0.72rem] uppercase tracking-[0.12em] text-brass">
                    {job.start}
                    <span className="text-ink-faint"> to </span>
                    {job.end}
                  </p>
                  <h3 className="display mt-3 text-[1.7rem] sm:text-[2rem]">{job.company}</h3>
                  <p className="mt-2 text-[0.95rem] font-medium text-ink-muted">
                    {job.role}
                    {job.location && <span className="text-ink-faint"> · {job.location}</span>}
                  </p>
                  <p className="mt-1 font-mono text-[0.7rem] text-ink-faint">{job.period}</p>
                </div>
              </div>

              <div className="lg:col-span-8">
                <p className="max-w-[62ch] text-pretty text-[1.02rem] leading-relaxed">{job.summary}</p>
                <ul className="mt-6 max-w-[64ch] space-y-3 text-[0.94rem] leading-relaxed text-ink-muted">
