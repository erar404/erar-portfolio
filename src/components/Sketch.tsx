  ...
  return (
    <figure className="frame gridpaper p-5 sm:p-7">
      <figcaption className="mb-6 flex items-center justify-between">
        ...
        <span className="label text-ink-faint">architecture sketch</span>
      </figcaption>
      ...
      <ol className="relative grid gap-3 sm:grid-cols-4 sm:gap-2">
        ...
        {nodes.map((n, i) => (
          ...
          >
            <span className="mb-1.5 block eyebrow">0{i + 1}</span>
            <span className="block font-display text-base font-semibold leading-tight tracking-tight">{n.label}</span>
            {n.sub && <span className="mt-1 block text-sm leading-snug text-ink-muted">{n.sub}</span>}
          </motion.li>
        ))}
      </ol>

      <ul className="mt-6 grid gap-x-6 gap-y-2 text-sm text-ink-muted sm:grid-cols-2">
        {notes.map((t) => (
